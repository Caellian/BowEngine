...
import com.caellian.bowEngine.core.util.helpers.DataUtil;
import com.caellian.bowEngine.render.resources.TextureResource;
...
public class Texture
{
	...
	{
		this(fileName, GL_NEAREST);
	}

	public Texture(@NotNull String fileName, int glScalingFilter) {
		...
		{
			resource = loadTexture(fileName, glScalingFilter);
			resourceHashMap.put(fileName, resource);
		}
	}

	public static TextureResource loadTexture(@NotNull String texture, int glScalingFilter)
	{
		...
		try
		{
			...
			glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, glScalingFilter);
			glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, glScalingFilter);
